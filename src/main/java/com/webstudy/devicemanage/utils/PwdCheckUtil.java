...
public class PwdCheckUtil {
    ...
    public static boolean checkByCustomRules(String password){
        // 包含大小写
        if(! (checkContainLowerCase(password) && checkContainUpperCase(password)))  return false;
        // 包含数字
        if(! checkContainDigit(password)) return false;
        // 相同连续字母不超过3位
//        if(! checkSequentialSameChars(password, 8))   return false;
        // 长度8-30位（等保三级）
        if(! checkPasswordLength(password, "8", "30")) return false;
        // 包含特殊符号
        if(! checkContainSpecialChar(password)) return false;
        // 键盘横向检测
//        if(! checkLateralKeyboardSite(password, 4, true)) return false;
        // 键盘斜向检测
//        if(! checkKeyboardSlantSite(password, 4, true)) return false;
